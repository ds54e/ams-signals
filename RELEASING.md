...
2. Decide the reuse policy. The owner may choose no license or different licenses for the site code, Golden factual compilation, and Analysis writing.
3. Review the PR metadata/history audit. The following prior branches have trees identical to their merged squash commits and can be deleted for repository hygiene:
   ...
   ```bash
   git push origin --delete bootstrap-v1 source-status-foundation research/apple-v1 research/wave1-v1 feat/visualization-v1 feat/analysis-v1
   ```
...
8. Confirm the deployed home page still contains `<meta name="robots" content="noindex, nofollow">` and manually inspect the Timeline, one Event-to-source path, the first Analysis-to-Event path, and a narrow viewport.
