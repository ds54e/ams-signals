...
access: "Paper, Hugging Face data, and model/evaluation scripts are public. Design code uses PySpice; model endpoints and separately arranged input/template files are required."
notice: "A public code repository is not a complete reproduction package. The paper defines pass@k as the fraction of k generated answers passing simulation, which differs from other benchmarks using that name."
addedAt: "2026-09-05"
...
Evaluator-side simulation should be distinguished from tools made available to the model. Reported experiments are not interchangeable with unrestricted tool-using agent runs.
