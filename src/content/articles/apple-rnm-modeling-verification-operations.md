...
さらにAMS関連の求人を追うと、PMUではDMS、Wirelessではサブシステム検証、その外側ではモデリング基盤やAMSシミュレーションの方法論まで出てきます。個々に見れば別々の求人ですが、並べるとAppleがRNMをどのように扱っているのかが少しずつつながってきます。

## モデルを書く人と、モデルを検証する人

「Mixed-Signal Behavioral Modeling Engineer」は、回路を単純にSystemVerilogへ書き直す仕事ではありません。2026年3月の求人では、必要な細かさを選びながら非理想性も取り込み、回路側から渡されたデータをPythonやMATLABで解析してモデルへ反映するとされています。モデルの性能評価、回路との一致確認、モデリング方法や機能カバレッジの改善までが対象で、PLL、DLL、SERDES、センサー、データコンバータなどの経験も挙げられていました。[[3]](#source-3)
...
モデル作成側にも一致確認は含まれているため、両者が単純に「実装」と「テスト」で分割されているわけではなさそうです。それでも、モデルを対象にテストベンチやアサーションを作り、検証フローそのものを整備する役割が独立している点は目につきます。

## PMUではDMSでループを閉じる

2026年4月23日の「Digital Mixed Signal Modeling Engineer」は、AppleのPMU DMSチームの求人です。対象はbuck converter、LDO、チャージポンプ、基準電圧回路などで、これらのアナログIPをSystemVerilog RNMとして実装します。モデルはデジタル制御回路と接続され、DMS上でループを閉じます。その目的として、シリコン前にデジタル側とアナログ側の両方から不具合を見つけることが明記されていました。[[5]](#source-5)
...
ここで中心になるのはモデルそのものではなく、無線のデジタル制御回路とRFサブシステムです。PMUの求人ではRNMの作成や精度管理まで担当していましたが、Wirelessではモデルが検証環境の構成要素として登場します。同じSystemVerilogのビヘイビアモデルを扱っていても、仕事の重心はかなり違います。
...
特に目につくのが「Mixed-Signal Model Verification Engineer」と「Analog Mixed-Signal Modeling Software Engineer」です。モデルを書く担当者自身も回路との一致を確認していますが、それとは別にモデル専用の検証フローを作る人がいて、多数の設計者がモデルを使うための基盤を作る人もいます。PMUでも精度指標、回帰テスト、モデルライブラリの保守までが職務に入っており、モデルを作った後の品質管理や運用にもかなりの仕事が割かれています。[[2]](#source-2) [[5]](#source-5) [[7]](#source-7)

2026年の求人を並べると、AppleではRNMが、個々の設計者が必要に応じて書くモデルという段階を越えつつあります。モデルの検証、再利用、変更追跡、継続運用まで含め、設計資産として扱う体制が見えてきました。RNMそのものより、その周囲にこれだけの役割が成立している点の方が、AppleのAMS検証がどの段階まで来ているかをよく表す材料です。
