...
async function viewerEventCounts(page) {
  const kinds = await page.locator('[data-events-json]').evaluate((node) => (
    JSON.parse(node.textContent).map((event) => event.kind)
  ));
  return {
    total: kinds.length,
    technical: kinds.filter((kind) => kind === 'technical').length,
    organizational: kinds.filter((kind) => kind === 'organizational').length,
  };
}
...
test('canonical JSON export contains the complete factual corpus', async ({ page }) => {
  ...
  expect(payload.companies).toHaveLength(66);
  expect(payload.people).toHaveLength(30);
  expect(payload.events).toHaveLength(215);
  ...
  expect(payload.events.filter(({ kind }) => kind === 'technical')).toHaveLength(147);
  expect(payload.events.filter(({ kind }) => kind === 'organizational')).toHaveLength(68);
...
test('Events is the chronological textual view without a Timeline or inspector', async ({ page }) => {
  ...
  const viewerCounts = await viewerEventCounts(page);
  await expect(page.locator('[data-status]')).toHaveText(`${viewerCounts.total} of ${viewerCounts.total} events`);
  await expect(page.locator('.event-filter-utility > .event-filter-summary'))
    .toHaveText(`${viewerCounts.total} of ${viewerCounts.total} events`);
...
test('Lunlun is a normal two-Event viewer trajectory that remains outside the factual export', async ({ page }) => {
  ...
  await expect(page.locator('[data-status]')).toHaveText(`2 of ${viewerCounts.total} events`);
...
test('Company picker is readable, searchable, and independently clearable', async ({ page }) => {
  for (const path of ['./', './events/']) {
    ...
    await expect(page.locator('[data-status]')).toHaveText(`0 of ${viewerCounts.total} events`);
    ...
    await expect(page.locator('[data-status]')).not.toHaveText(`0 of ${viewerCounts.total} events`);
    expect(new URL(page.url()).searchParams.has('companies')).toBe(false);
...
test('Timeline utility bar places count and legend beside the compact controls', async ({ page }) => {
  ...
  expect(representedIds.length).toBeLessThan(viewerCounts.total);
  await expect(summary.locator(':scope > .index-count'))
    .toHaveText(`${representedIds.length} of ${viewerCounts.total} events`);
    ...
    for (const viewValue of ['companies', 'people', 'both']) {
      ...
      if (surface === 'timeline') {
        ...
        await expect(page.locator('[data-status]'))
          .toHaveText(new RegExp(`of ${viewerCounts.total} events`));
      }
    }
...
test('Timeline always shows both Signal types while Events retains kind filtering', async ({ page }) => {
  ...
  const totalCount = technicalCount + organizationalCount;
  expect(serializedKinds).toHaveLength(totalCount);
  ...
  for (const kindValue of ['technical', 'organizational']) {
    ...
    await expect(page.locator('[data-kind]')).toHaveCount(0);
    await expect(page.locator('[data-status]')).toHaveText(`${representedCount} of ${totalCount} events`);
  ...
  await expect(page.locator('[data-status]')).toHaveText(`${technicalCount} of ${totalCount} events`);
  ...
  await expect(page.locator('[data-status]')).toHaveText(`${organizationalCount} of ${totalCount} events`);
