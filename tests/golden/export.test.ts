...
import {
  buildExportPayload,
  EXCLUDED_EXPORT_EVENT_IDS,
  EXCLUDED_EXPORT_PERSON_IDS,
} from '../../src/lib/export.ts';

async function readEntries(relativeDirectory: string) {
...
test('canonical JSON export contains the complete factual corpus', async () => {
  const [companyEntries, peopleEntries, eventEntries] = await Promise.all([
    readEntries('src/data/companies'),
    readEntries('src/data/people'),
    readEntries('src/data/events'),
  ]);
  ...
  const sourceEvents = eventEntries
    .map(({ data }) => data)
    .filter(({ id }) => !EXCLUDED_EXPORT_EVENT_IDS.has(id));
  ...
  expect(payload.people).toHaveLength(peopleEntries.filter(({ data }) => !EXCLUDED_EXPORT_PERSON_IDS.has(data.id)).length);
  expect(payload.events).toHaveLength(sourceEvents.length);
  ...
  expect(payload.events.filter(({ kind }) => kind === 'technical')).toHaveLength(sourceEvents.filter(({ kind }) => kind === 'technical').length);
  expect(payload.events.filter(({ kind }) => kind === 'organizational')).toHaveLength(sourceEvents.filter(({ kind }) => kind === 'organizational').length);
