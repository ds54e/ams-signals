...
Current presentation refinement: 2026-09-05, based on `main` commit `688bfcf`. The original landscape/source review below was completed on `analog-ai-landscape-redesign` from `4fa73e4650d7512d236004297aeb8be9a5cf4296`.

## Current structure and behavior

- `src/content/analog-ai/*.md`: thirteen independent English entries. All existing metadata, Markdown research and source IDs remain unchanged. A new bounded `description` condenses each existing research record into a concrete use-case/capability paragraph. No project claims, classifications, activity counts, repositories or review dates were re-researched or changed in this presentation pass.
- `src/pages/analog-ai/index.astro` and `src/styles/analog-ai.css`: a semantic six-column Landscape followed by a compact four-area index, both sorted by newest public activity. No visible title, introduction, count, additions, snapshot/review dates, methodology or reproduction prose. Moderately larger typography and a bold latest activity date support scanning. Mobile stacks each entry; only the matrix scrolls horizontally and its project column stays sticky.
- `src/lib/analog-ai/catalog.ts`: one immutable sort uses GitHub `lastCommitAt` or no-repository `lastPublicUpdateAt`, descending. Ties use NFKC-normalized lower-case name then slug; missing dates sort last. `browser.ts` remains unchanged: native history and links reveal real project/descendant targets and open their disclosure, including repeated activation of the current permalink.
...
- Native **What it does** disclosures are closed in server-rendered HTML. They show one concise capability paragraph and the original primary-source links, not the old research/prerequisite/activity prose. Core information works without JavaScript. Legacy Markdown heading IDs are retained as small, non-text anchors at the new paragraph so old detail bookmarks still open useful content; source IDs still point to their original links. This repairs the missing detail targets in the inherited `688bfcf` presentation.
...
## Dashboard refinement verification
