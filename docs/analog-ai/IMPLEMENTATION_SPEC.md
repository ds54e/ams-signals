...
Do not repeat project names in a separate activity overview. Desktop rows have four information areas: project/description, keywords, activity, and roles/links. Use approximately `2.2fr / 1.05fr / 1.2fr / 0.55fr`, giving the description the most space. Aim for 4–6 entries in a normal desktop viewport when viewing the index. On mobile stack the same four areas naturally. Use readable text without oversized cards or excessive whitespace. Remove project counts, recent additions, review/snapshot metadata, A–Z labels, activity explanations, window/dot legends, methodology and reproduction paragraphs from the browsing surface. Keep maintenance documentation in the repository.
