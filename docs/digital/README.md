...
Project names are plain text. On the same left-aligned wrapping title line, show only the name and available Website / Paper / Code / Results links, in that order. A single concrete description follows. Scope stages (Design, Synthesis, Verification, Layout) appear as compact, vertically stacked uppercase category labels in the metadata rail. No self-permalink links or fragment compatibility aliases are maintained.

The metadata rail shows a compact, normal-weight date including its year (`Sep 5, 2026`), twelve binary cells with the newest month at the left and oldest at the right, then Scope labels separated by a compact 10px gap. Month totals remain non-visible; no extra labels or section headings separate this block. Repository-backed records use reviewed monthly history from their canonical host, including GitLab. Paper/release/public-update records activate the reviewed event month without invented commit counts. Provenance is available through hover/accessibility metadata rather than a visible date prefix. Activity describes recorded public signals, never total development effort or quality.
