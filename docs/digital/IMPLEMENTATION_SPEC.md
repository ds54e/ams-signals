...
`CatalogIndex.astro` supplies the same small native form to both domains; `src/scripts/catalog-filter.ts` enhances it. Search is about **300px**, the stage select **150px**, and the quiet normal-weight count aligns right. At 600px and below Search takes a full row, with Scope and count below. Shared `filters.css` owns native control styling; there is no outer card, toolbar rule, reset button or replacement overview. First-row padding supplies about 22px of whitespace below the controls.
