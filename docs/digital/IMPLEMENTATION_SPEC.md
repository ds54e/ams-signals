...
Use the shared **920px listing width** aligned with Articles and Events. Each article has two columns: a **150px metadata rail**, **24px gap**, and **746px flexible project body** at full width. No visible column headers, repeated Scope/Activity labels, cards or standalone legend. At 760px and below the project body comes first, followed by the rail; both retain full readable width without horizontal scrolling.

The rail contains exactly this order: latest date, twelve activity ticks, Scope labels. The strip-to-Scope gap is **4px**, with no blank line or visible month count. Each Scope label is a fit-content rectangle with **12px/500** text, **1.35** line height, **2px × 6px** padding, **3px radius** and **3px vertical gaps**. Design is green/teal; Simulation and Verification blue; Synthesis ochre; Layout rust; AI-built violet. AI-prefixed stages use their base stage's color. Text supplies category meaning in monochrome; forced colors uses system foreground/background and a visible outline. The shared [visual system](../VISUAL_SYSTEM.md) owns the palette and layout details.

The project body has an accessible plain-text H2 followed immediately by authored Website, Paper, Code, Results links in that order. The title line is baseline-aligned, wrapping and left aligned with **12px gaps**. No self-links, aliases, classification badges or replacement icons appear beside the name. A single concise capability description follows; preserve useful technical identifiers naturally. Source URLs remain unchanged.
