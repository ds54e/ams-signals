...
Only **Project | Scope | Activity**. The column-header row is the first visible catalog content. Catalog max width is **1120px**, centered within the existing page shell; no global width change. Desktop columns are **`minmax(0, 1fr) 170px 112px`**, with **22px gaps**. At full width Project is **794px**. Below 900px stack the three areas in document order and hide the desktop header. No horizontal scrolling, oversized cards or extra prose.
...
The shared index primitives provide 17px/700 titles, 15px muted descriptions at 1.65 line height, 22px/24px row padding and 9px title-to-description spacing. Quick links and Scope use 13px; dates and activity summaries use 12px normal-weight metadata. These tokens also govern Articles and Events, without forcing their outer widths to match Catalog.
