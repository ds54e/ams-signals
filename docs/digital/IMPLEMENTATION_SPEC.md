...
The rail contains exactly this order: latest date, twelve activity ticks, Scope labels. The strip-to-Scope gap is **6px**, with no blank line or visible month count. Each Scope label is a fit-content rectangle with **11px/500** text, **1.35** line height, **2px × 5px** padding, **2px radius** and **3px vertical gaps**. Design is green/teal; Simulation and Verification blue; Synthesis ochre; Layout rust; AI-built violet. AI-prefixed stages use their base stage's color. Text supplies category meaning in monochrome; forced colors uses system foreground/background and a visible outline. The shared [visual system](../VISUAL_SYSTEM.md) owns the palette and layout details.
