...
Strict schema: `name`, optional `aliases`, one English single-paragraph `description` (600 characters), required `flow`, `access`, valid `addedAt`/`reviewedAt`, and `sources`. Review cannot precede addition.
...
`validate:digital` and `test:digital` run inside `npm run check`. Validate stable unique IDs, strict nonempty Flow and supported states, concise descriptions, source URLs/IDs/purposes, complete activity coverage, valid consecutive months/counts/dates, canonical identities and inclusive meaningful freshness. Keep all existing activity, Golden and companion-catalog checks.
