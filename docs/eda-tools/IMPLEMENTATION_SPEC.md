...
The index begins with column labels, not a section heading. Desktop proportions are approximately `2.2fr / 1.05fr / 1.2fr / 0.55fr`. Mobile stacks description, keywords, activity, type/links. Keep one compact role string in authored order, joined with ` + `, appending ` · AI-built` only for explicitly reviewed AI-built entries. Never display primary category, AI-enabled or Traditional as public type labels. Keep Website / Paper / Code / Results links when sourced. Do not show full research sources, prerequisites or notes. Native hash anchors identify focusable project rows; no client JavaScript is needed for permalinks, reload or history.
