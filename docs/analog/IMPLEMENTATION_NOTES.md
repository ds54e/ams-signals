...
Validation: `npm run check` passes, including **23 Analog / 24 Digital** unit tests, Golden checks, the **285-page** build and **2,976** internal anchors. The full production-preview Chromium suite passes **91/91**, covering both catalog filters, no-JS, keyboard/IME input, all-row activity provenance, forced colors, badges, Timeline hit areas/selection and existing viewer behavior. Final screenshots cover **1440, 1280, 1024, 390 and 320px**, filter/empty states, Japanese Articles, company/person views and dark mode, without page-level overflow. All **352 content/data files** remain byte-identical to the starting commit; `/export.json` retains SHA-256 `67586997053b77e6215c53ce12188a5013d0bb6b1e0411370570c67a94bd1aeb`. Deployment uses the existing manual Pages workflow, followed by production browser and artifact verification.
...
`npm run check` passes: both catalog validators, **Analog 22/22** and **Digital 24/24** unit tests, Golden validation/fact lint/duplicate checks, the **285-page** build and **2,976** internal links. Explicit unit commands also pass. The complete Chromium production-preview suite passes **70/70**, including per-entry stage/AI rendering, filled/open accessibility, no-JS/keyboard use, normal-weight dates, genuine repository versus point-event bands, navigation, noindex and viewer isolation. The geometry assertions require real Scope items and check vertical spacing rather than accepting an empty selector.

Visually inspected both catalogs at **1440, 1280, 1024, 390 and 320px**. Conventional simulators/layout tools, AI design/simulation/verification projects, AI-built tools, first/middle/last and multi-link rows remain readable without overlap or page-level overflow. Project dominates; Scope and Activity stay secondary. No replacement overview or visible methodology is present.

Compared with the starting commit, all **68** project identities, descriptions, non-Scope metadata and source arrays are identical; the reviewed stage levels are preserved after individual reassessment. Both activity snapshots and **284** other content/data files are byte-identical. All **283** non-catalog generated HTML pages also match exactly. `/export.json` retains SHA-256 `67586997053b77e6215c53ce12188a5013d0bb6b1e0411370570c67a94bd1aeb`. No obsolete active taxonomy or project-wide AI enum is retained. Existing aliases and research/access metadata remain for durable identity and provenance, without adding public controls.

Browser automation is Chromium-only; external EDA/benchmark claims were not independently reproduced. Deployment uses the existing manual Pages workflow only when authorized, followed by the same production smoke suite and content/export checks.

## AI classification judgments

- **ZeroSim** is AI Simulation: the learned transformer produces the performance estimate. **AnalogCoder-Pro**, **ATLAS** and **Masala-CHAI** additionally have model-driven waveform/testbench/error interpretation. Masala's optional simulator path stays supporting. ATLAS is paper-reported and remains without Layout.
- **AnalogSAGE**, **ASTRA**, **AutoSizer**, **ARCS**, **G-DiffPS**, **PANDA** and **virtuoso-agent** use AI in topology/sizing decisions. Their reviewed numerical simulation backends stay unprefixed. Feedback to a design policy is not automatically a second AI stage. PANDA's reviewed placement/routing kernels also stay conventional Layout.
- **EvoLDO-Bench** is AI Design for its model reasoning runner, with supporting AI Simulation only for the separate controlled tool track: models fix measurement/configuration problems and diagnose evidence-bound failures. The no-tool reasoning track stays distinct. **Razavi-Bench** exposes model reasoning but its optional numerical simulator treatment stays unprefixed.
- **CircuitRubric**, **NetlistBench**, **LOADBench**, **AnalogDesignBench**, **AnalogGym** and **vaBench** describe benchmark/data/environment operations. Grading model submissions or providing a container for a supplied agent does not itself establish an AI stage in those released tools.
- **AnalogForge** stays unprefixed: the inspected workflow uses bounded policies and numerical/analytic fixtures; the README identifies LLM+BO as a proxy. A standalone provider adapter is insufficient to label the actual design workflow AI.
- **AMS-IO-Agent** is AI Design for structured intent/schematic generation; the reviewed T28 geometry/SKILL layout generator stays unprefixed. **vcli**, **virtuoso-bridge-lite** and **gmoverid-skill** expose tools and do not acquire AI stage prefixes just from agent-oriented interfaces/instructions.
- Only **Ngspice + OpenVAF Enhancements** has defining AI-built on Analog, supported by author documentation and core compiler development. Its numerical Simulation is not AI Simulation and its features are not automatically upstream.
...
1. Review primary sources before changing prose or scope. Keep stable filenames, reviewed Scope stages, and `core|supporting` only; omit unreviewed/future scope. Do not create Golden records.
