...
The shared model is optional `scope.aiDevelopment: assisted | built`, paired with `developmentEvidence` (factual summary, existing source IDs and independent review date). Legacy `aiBuilt` is rejected. Scope stages, runtime booleans, functional descriptions and activity snapshots remain unchanged; primary navigation was unchanged by that migration (Articles left the primary navigation later, in the post-cutover indexing change). Shared outline/filled muted-red badges follow the stages with an 8px separation and open inline evidence through native buttons; static HTML includes the same explanation and links. Search uses only existing public project text plus the exact visible labels.
...
- **ZeroSim** is AI Simulation: the learned transformer produces the performance estimate. **AnalogCoder-Pro**, **ATLAS** and **Masala-CHAI** additionally have model-driven waveform/testbench/error interpretation. Masala's optional simulator path remains a documented V2 operation. ATLAS is paper-reported and remains without Layout.
...
- Only **Ngspice + OpenVAF Enhancements** has AI-built on Analog, supported by author documentation and core compiler development. Its numerical Simulation is not AI Simulation and its features are not automatically upstream.
...
1. Review primary sources before changing prose or scope. Keep stable filenames, reviewed stage presence and explicit AI flags; omit incidental/unreviewed/future scope. Do not create Golden records.
