...
Project names are plain text. On the same left-aligned wrapping title line, show only the name and available Website / Paper / Code / Results links, in that order. A single concrete description follows. Flow stages (Design, Simulation, Layout) appear vertically, one per line, with small filled/open circles for core/supporting scope. No self-permalink links or fragment compatibility aliases are maintained.
