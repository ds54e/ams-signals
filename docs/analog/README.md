...
Project names are plain text. On the same left-aligned wrapping title line, show only the name and available Website / Paper / Code / Results links, in that order. A single concrete description follows. Scope stages (Design, Simulation, Layout) appear as compact, vertically stacked uppercase category labels in the metadata rail. No self-permalink links or fragment compatibility aliases are maintained.
