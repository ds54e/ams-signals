...
Global Activity Matrix time runs linearly from newest-left to oldest-right with sparse year ticks derived from the full corpus. The Matrix prioritizes broad activity-pattern readability over day-level visual precision: Events that are close in time may be compacted into deterministic visual bundles, while each bundle member retains its individual Technical or Organizational shape. Underlying Event timing remains factual and unchanged, and exact dates remain available through the Evidence Inspector and Events view. Bundling is presentation-only; it creates no new Golden entity or export field. Company and Person detail pages retain the existing segmented, chronologically packed Timeline geometry.
